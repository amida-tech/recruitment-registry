...
const makeVicinityRequest = function makeVicinityRequest(zip, page, customDistance) {
    const format = 'json';
    const radius = customDistance ? customDistance : zipConfig.distance || 50;
    const key = zipConfig.apiKey;

    console.log('>>>>> zip-util > makeVicinityRequest > radius: ', radius);
...
const runVicinity = function runVicinity(zip, page = 0, customDistance) {
    console.log('>>>>> zip-util > runVicinity > customDistance: ', customDistance);
...
const findVicinity = function findVicinity(zip, customDistance) {
    console.log('>>>>> zip-util > findVicinity > customDistance: ', customDistance);
