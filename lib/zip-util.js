...
const parseVicinityResponse = function parseVicinityResponse(result, page, zip) {
    ...
    const zips = result.body.results.map(result => formatZip(result.zip));
...
const runVicinity = function runVicinity(zip, page = 0) {
    return requestGet(makeVicinityRequest(zip, page))
        .then(result => parseVicinityResponse(result, page, zip), err => RRError.reject('zipApiError', err.code, err.message));
};
/* jshint +W003 */
