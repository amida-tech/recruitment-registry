...
exports.addSubqueries = function (query, sqTemplate, separator, sqReplacements) {
    ...
    const subqueries = sqReplacements.map((replacementMap, sqIdx) => {
        ...
        for (let replacement in replacementMap) {
            const replacementKey = replacement + "_idx_" + sqIdx;
            subquery = subquery.split(':' + replacement).join(':' + replacementKey); // replace all
            replacements[replacementKey] = replacementMap[replacement];
        }
        return subquery;
    });
