...
class RRError extends Error {
    ...
    getMessage(i18n) {
        ...
        let msg = i18n.__({ phrase: this.code, locale }, i18nParams); // eslint-disable-line no-underscore-dangle
        if (msg === this.code && locale !== 'en') {
            msg = i18n.__({ phrase: this.code, locale: 'en' }, i18nParams); // eslint-disable-line no-underscore-dangle
        }
        if (msg === this.code) {
            msg = i18n.__({ phrase: 'unknown', locale }, i18nParams); // eslint-disable-line no-underscore-dangle
        }
        return msg;
    }
