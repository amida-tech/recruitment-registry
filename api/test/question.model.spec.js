...
describe('question unit', function () {
    ...
    it('get all questions when none', function () {
        return dao.question.listQuestions()
            .then(questions => {
    ...
    const qxBasicFn = function () {
        return function () {
            ...
            return dao.question.createQuestion(clientQuestion)
                .then(id => dao.question.getQuestion(id))
                .then(question => {
                ...
                .then(question => {
                    ...
                    return dao.question.updateQuestionText({ id: question.id, text })
                        .then(() => question);
                })
                .then(question => dao.question.getQuestion(question.id))
                .then(updatedQuestion => {
                    ...
                    return dao.question.updateQuestionText({ id: updatedQuestion.id, text });
                });
        };
    };
    ...
    it('error: get with non-existent id', function () {
        return dao.question.getQuestion(99999)
    ...
    it('get multiple questions (2, 4, 7)', function () {
        ...
        return dao.question.listQuestions({ ids })
    ...
    it('error: get multiple with non-existent id', function () {
        return dao.question.listQuestions({ ids: [1, 99999] })
            .then(shared.throwingHandler, shared.expectedErrorHandler('qxNotFound'));
    });
    ...
    const translateQuestionFn = function (index, language) {
        return function () {
            ...
            return dao.question.updateQuestionText(translation, language)
                .then(() => {
    ...
    const getTranslatedQuestionFn = function (index, language) {
        return function () {
            ...
            return dao.question.getQuestion(id, { language })
                .then(result => {
    ...
    const listTranslatedQuestionsFn = function (language) {
        return function () {
            return dao.question.listQuestions({ language })
                .then(result => {
    ...
    const qxDeleteFn = function (index) {
        return function () {
            return dao.question.deleteQuestion(hxQuestion.id(index))
                .then(() => {
    ...
    it('delete survey 2', function () {
        return dao.survey.deleteSurvey(hxSurvey.id(2))
    ...
    it(`error: replace a non-existent question`, function () {
        ...
        return dao.question.replaceQuestion(999, replacement)
            .then(shared.throwingHandler, shared.expectedErrorHandler('qxNotFound'));
    });
    ...
    _.forEach([2, 7, 9], questionIndex => {
        it(`error: replace question ${questionIndex} on an active survey`, function () {
            const replacement = generator.newQuestion();
            return dao.question.replaceQuestion(hxQuestion.id(questionIndex), replacement)
                .then(shared.throwingHandler, shared.expectedErrorHandler('qxReplaceWhenActiveSurveys'));
        });
    });

    it('delete survey 0', function () {
        return dao.survey.deleteSurvey(hxSurvey.id(0))
    ...
    it('delete survey 3', function () {
        return dao.survey.deleteSurvey(hxSurvey.id(3))
    ...
    [7, 10, 14, 21, 22, 24].forEach((questionIndex, index) => {
        it(`replace question ${questionIndex} with question ${20 + index}`, function () {
            ...
            return dao.question.replaceQuestion(hxQuestion.id(questionIndex), replacement)
                ...
                .then(() => dao.question.listQuestions())
                .then(questions => comparator.questions(hxQuestion.listClients(), questions));
        });
    });
